...
6. **Wiki update** *(mandatory for every feature commit)*: Update the corresponding wiki page(s) in `src/lib/wikiDefaults.ts` whenever you add or modify a user-facing feature. The wiki is the canonical user-facing documentation — treat it as part of the definition of done. If you add a new feature, add a new wiki page for it. If you modify an existing feature, update the relevant page. The wiki content uses Markdown format.
